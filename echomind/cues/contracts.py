...
class RenderedStimulus(BaseModel):
    ...
    def validate_rendered_shape(self) -> RenderedStimulus:
        ...
        if self.delivery_mode == CueDeliveryMode.TEXT_ONLY and not self.text_payload:
            raise ValueError("text_only stimulus requires text_payload")

        if self.delivery_mode == CueDeliveryMode.NARRATION and not self.narration_transcript:
            raise ValueError("narration stimulus requires narration_transcript")

        if self.delivery_mode == CueDeliveryMode.SLIDESHOW_NARRATION:
            if not self.narration_transcript:
                raise ValueError("slideshow stimulus requires narration_transcript")
